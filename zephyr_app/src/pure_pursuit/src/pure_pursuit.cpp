// Based on: https://gitlab.com/autowarefoundation/autoware.auto/AutowareAuto/-/blob/e3e26be1ab4b822996df60f261d031d7924f20ac/src/control/pure_pursuit/src/pure_pursuit.cpp
...
VehicleControlCommand PurePursuit::compute_command_impl(const TrajectoryPointStamped & current_pose)
{
  ...
  if (is_success) {
    ...
    m_command.velocity_mps =
      get_reference_trajectory().points._buffer[get_current_state_spatial_index()
...
void PurePursuit::compute_errors(const TrajectoryPoint & current_point)
{
  ...
  if (traj.points._length == 0) {
  ...
  } else if (traj.points._length == 1U) {
    target = traj.points._buffer[0U];
  } else {
    ...
    if (0U != first_idx_after_current) {
      const auto & next_point = traj.points._buffer[first_idx_after_current];
      ...
      const auto & last_point = traj.points._buffer[last_idx];
    ...
    const auto & nearest_point = traj.points._buffer[nearest_idx];
    const auto & second_point = traj.points._buffer[second_nearest_idx];
  ...
  const uint32_t idx)
{
  ...
  const TrajectoryPoint & prev_target_point =
    has_idx ? get_reference_trajectory().points._buffer[prev_idx] : current_point;
...
bool8_t PurePursuit::compute_target_point(const TrajectoryPoint & current_point)
{
  ...
  const auto & traj = get_reference_trajectory();
  for (; idx < traj.points._length; ++idx) {
    const TrajectoryPoint & target_point = traj.points._buffer[idx];
  ...
  if (idx == traj.points._length) {
    if (is_travel_direct) {
      // use the farthest target index in the traveling direction
      m_target_point = traj.points._buffer[last_idx_for_noupdate];
    } else {
